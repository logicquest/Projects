...

{
	...
	for (j = 0; j<network->output_neuron_count; j++)
	{
		for (k = 0; k < network->hidden_neuron_count; k++){
			...
			network->second_layer_weights[j*network->hidden_neuron_count + k] += (network->second_layer_delta_weigts[j*network->hidden_neuron_count + k] /16);
			network->second_layer_delta_weigts[j*network->hidden_neuron_count + k] = 0.0;
		}
		network->output_layer[j].bias += (network->second_layer_delta_bias[j] / 16);
	...
	for (j = 0; j<network->hidden_neuron_count; j++)
	{
		...
		network->first_layer_weights[j*network->input_node_count + k] += (network->first_layer_delta_weigts[j*network->input_node_count + k] / 16);
		...
		network->hidden_layer[j].bias += (network->first_layer_delta_bias[j] / 16);
