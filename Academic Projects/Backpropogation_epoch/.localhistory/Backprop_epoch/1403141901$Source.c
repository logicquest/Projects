...

{
	...
	for (j = 0; j < network->hidden_neuron_count; j++)
	{
		for (k = 0; k < network->input_node_count; k++){
			...
			printf("%f", network->first_layer_delta_weigts[j*network->hidden_neuron_count + k]);
		}
	}
	...
	for (j = 0; j<network->hidden_neuron_count; j++)
	{
		for (k = 0; k < network->input_node_count; k++){
			...
			network->first_layer_weights[j*network->input_node_count + k] += (network->first_layer_delta_weigts[j*network->hidden_neuron_count + k] / 64);
...
int main()
{
	...
	*/
	...
	//	print_network_parameters(my_network);
		...
		for (int count = 0; count < (my_network->input_node_count * my_network->hidden_neuron_count); count++)
		{
			printf("%f\n", my_network->first_layer_delta_weigts[count]);
		}
...
void compute_delta_weights(network_ptr my_network,double learning_rate)
{
	...
	for (j = 0; j < 20; j++)
	{
		printf("%f\n number\n", my_network->first_layer_delta_weigts[j]);
	}
	...
	for (j = 0; j<my_network->hidden_neuron_count; j++)
	{
		for (k = 0; k < my_network->input_node_count; k++){
			...
			printf("%f\n Indices\n", my_network->first_layer_delta_weigts[j*my_network->hidden_neuron_count + k]);
	...
	printf("%f", my_network->first_layer_delta_weigts[j*my_network->input_node_count + k]);
	for (j = 0; j<my_network->hidden_neuron_count; j++)
	{
		for (k = 0; k < my_network->input_node_count; k++){
		...
		printf("%f\n Gradient \n  %f \n Input Layer \n %f \n", my_network->first_layer_delta_weigts[j*my_network->hidden_neuron_count + k], my_network->hidden_layer[j].gradient, my_network->input_layer[k]);
