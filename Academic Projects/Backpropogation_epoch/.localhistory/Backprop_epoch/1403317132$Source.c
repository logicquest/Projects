...

{
	...
	printf("deltawts %f ", network->first_layer_weights[0]);
...

{
	...
	for (j = 0; j<network->hidden_neuron_count; j++)
	{
		for (k = 0; k < network->input_node_count; k++){
			...
			printf("weigts%f\n deltawts %f ", network->first_layer_weights[j*network->input_node_count + k], (network->first_layer_delta_weigts[j*network->input_node_count + k] / 64));
...
void compute_delta_weights(network_ptr my_network,double learning_rate)
{
	...
	for (j = 0; j<my_network->hidden_neuron_count; j++)
	{
		for (k = 0; k < my_network->input_node_count; k++){
			...
			printf("deltawts %f \n", (my_network->first_layer_delta_weigts[j*my_network->input_node_count + k] / 64));
		}
