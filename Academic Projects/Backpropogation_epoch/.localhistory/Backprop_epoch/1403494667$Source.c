...
#include <rng.h>
#include "C:\Users\\Chandrasekar\\Downloads\\Prasad Lectures\Slides\\Homework_03_Code\Homework_03_Code\\foo.c"
#include "C:\Users\\Chandrasekar\\Downloads\\Prasad Lectures\Slides\\Homework_03_Code\Homework_03_Code\\bar.c"
#include "C:\Users\\Chandrasekar\\Downloads\\Prasad Lectures\Slides\\Homework_03_Code\Homework_03_Code\\baz.c"
#include "C:\Users\\Chandrasekar\\Downloads\\Prasad Lectures\Slides\\Homework_03_Code\Homework_03_Code\\bonk.c"
...

{
	*network = (network_t*)malloc(sizeof(network_t));
	...
	(*network)->first_layer_delta_bias = (double*)malloc(sizeof(double)*hidden_nodes);
...

{
	...
	free((*network)->first_layer_delta_bias);
	free((*network)->second_layer_delta_bias);
	free(*network);
	*network = NULL;
}
...

{
	...
	for (k = 0; k<network->hidden_neuron_count; k++)
	{
		...
		network->hidden_layer[k].gradient = network->hidden_layer[k].output *(1.0 - network->hidden_layer[k].output) *gradient_accumulator;
	}
...

{
	...
	for (j = 0; j<network->output_neuron_count; j++)
	{
		for (k = 0; k < network->hidden_neuron_count; k++){
			...
			network->second_layer_weights[j*network->hidden_neuron_count + k] += (network->second_layer_delta_weigts[j*network->hidden_neuron_count + k] /val);
			network->second_layer_delta_weigts[j*network->hidden_neuron_count + k] = 0.0;
		}
		network->output_layer[j].bias += (network->second_layer_delta_bias[j] / val);
	...
	for (j = 0; j<network->hidden_neuron_count; j++)
	{
		...
		network->first_layer_weights[j*network->input_node_count + k] += (network->first_layer_delta_weigts[j*network->input_node_count + k] / val);
			
		network->first_layer_delta_weigts[j*network->input_node_count + k] = 0.0;

			
		}


		network->hidden_layer[j].bias += (network->first_layer_delta_bias[j] / val);
...
int main()
{
	...
	malloc_network(2, 10, 2, &my_network); /* Two input nodes, two hidden nodes, one output node */
	...
	*/
	...
	for (c = 0; c < 5000; c++)
		{
			...
			compute_network_outputs(foo[c], my_network);                     /* feed inputs forward  */
			compute_network_gradients(D[0], my_network);							/* compute gradients       */
			compute_delta_weights(my_network, 0.3);
		}
	...
	for (c = 0; c <5000; c++)
		{
	compute_network_outputs(bar[c], my_network);                     /* feed inputs forward  */
	...
	for (c = 0; c < 5000; c++)
{
		compute_network_outputs(baz[c], my_network);                     /* feed inputs forward  */
	...
	for (c = 0; c < 5000; c++)
	{
		compute_network_outputs(bonk[c], my_network);                     /* feed inputs forward  */
		compute_network_gradients(D[3], my_network);							/* compute gradients       */
		compute_delta_weights(my_network, 0.3);
	}
		

		apply_delta_rule(my_network, 0.3);
		//printf("%f\n%f\n", my_network->output_layer[0].gradient, my_network->output_layer[1].gradient);
		error_allowed = 0.000005;
...
void compute_delta_weights(network_ptr my_network,double learning_rate)
{
	...
	for (j = 0; j<my_network->hidden_neuron_count; j++)
	{
		for (k = 0; k < my_network->input_node_count; k++){
			...
			//printf("%f\n\n%f", my_network->hidden_layer[j].gradient, my_network->input_layer[k]);
		}
