...

{
	...
	for (j = 0; j<network->output_neuron_count; j++)
	{
		for (k = 0; k < network->hidden_neuron_count; k++){
			...
			network->second_layer_weights[j*network->hidden_neuron_count + k] += (learning_rate *(network->second_layer_delta_weigts[j*network->hidden_neuron_count + k] / 64));
			network->second_layer_delta_weigts[j*network->hidden_neuron_count + k] = 0.0;
		}
		network->output_layer[j].bias += (learning_rate *(network->second_layer_delta_bias[j] / 64));
	...
	for (j = 0; j<network->hidden_neuron_count; j++)
	{
		...
		network->first_layer_weights[j*network->input_node_count + k] += (learning_rate *(network->first_layer_delta_weigts[j*network->input_node_count + k] / 64));
			
		network->first_layer_delta_weigts[j*network->input_node_count + k] = 0.0;	
		}
		
		network->hidden_layer[j].bias += (learning_rate * (network->first_layer_delta_bias[j] / 64));
...
int main()
{
	...
	*/
	...
	do
	{
		...
		apply_delta_rule(my_network, 0.05);
