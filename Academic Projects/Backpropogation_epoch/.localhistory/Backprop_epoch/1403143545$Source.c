...
void compute_delta_weights(network_ptr my_network,double learning_rate)
{
	...
	for (j = 0; j<my_network->hidden_neuron_count; j++)
	{
		for (k = 0; k < my_network->input_node_count; k++){
			...
			if (j*my_network->input_node_count + k == 5)
			{
				somevalue = 1;
			}
			...
			printf("%f\t%f", my_network->first_layer_delta_weigts[j*my_network->input_node_count + k], (learning_rate *	my_network->hidden_layer[j].gradient *	my_network->input_layer[k]));
